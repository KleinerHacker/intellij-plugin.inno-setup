...
/**
 * Tests that [org.pcsoft.intellij.plugin.inno_setup.preprocessor.language.parser.IsPreprocessorAnnotator] validates the directive keyword against the ISPP spec
 * ([org.pcsoft.intellij.plugin.inno_setup.preprocessor.services.IsPreprocessorService], the single source of truth).
 */
class IsPreprocessorAnnotatorDirectiveTest : IsTimedBasePlatformTestCase() {
    ...
    fun testBuiltinFunctionReferenceNoError() {
        val errors = errorsContaining(
            "#define A GetFileVersionString(\"app.exe\")\n$setupTail",
    ...
    fun testMacroParametersDoNotProduceUnresolvedError() {
        ...
        val errors =
            errorsContaining("#define Max(a, b) a > b ? a : b\n$setupTail", "Unresolved preprocessor reference")
    ...
    fun testNonParameterIdentifierInMacroBodyProducesError() {
        ...
        assertTrue(
            "Error should name the unresolved identifier",
            errors.any { it.description?.contains("Helper") == true })
    }
