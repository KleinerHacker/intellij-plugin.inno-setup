...
/**
 * Tests for `#if`/`#elif` condition-expression analysis in [org.pcsoft.intellij.plugin.inno_setup.preprocessor.language.parser.IsPreprocessorAnnotator]: type errors,
...
class IsPreprocessorConditionalExpressionTest : IsTimedBasePlatformTestCase() {
    ...
    fun testTypeErrorInIfConditionIsMarked() {
        ...
        val errors =
            myFixture.also { it.configureByText(IsScriptFileType.INSTANCE, "#if \"a\" * \"b\"\n#endif\n$setupTail") }
                .doHighlighting()
                .filter { it.severity.name == "ERROR" }
        assertTrue(
            "A type error in an #if condition must be marked, was: ${errors.map { it.description }}",
    ...
    fun testDefinedArgumentIsNotUnresolved() {
        ...
        assertTrue(
            "defined(Maybe) must not flag 'Maybe' as unresolved, was: ${errors.map { it.description }}",
            errors.isEmpty()
        )
    }
