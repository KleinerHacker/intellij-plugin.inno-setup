...
/**
 * Tests that [org.pcsoft.intellij.plugin.inno_setup.preprocessor.language.parser.IsPreprocessorAnnotator] validates the `#pragma` directive: the sub-command must be known and
...
class IsPreprocessorPragmaAnnotatorTest : IsTimedBasePlatformTestCase() {
    ...
    fun testMissingSubCommandProducesError() {
        assertTrue(
            "A bare #pragma must be flagged",
            errorsContaining("#pragma\n", "requires a sub-command").isNotEmpty()
        )
    }
    ...
    fun testInvalidFlagLetterProducesError() {
        assertTrue(
            "Unknown flag letter must be flagged",
            errorsContaining("#pragma option -x+\n", "flag letter").isNotEmpty()
        )
    }

    fun testMalformedFlagProducesError() {
        assertTrue(
            "Flag without sign must be flagged",
            errorsContaining("#pragma option -v\n", "Invalid #pragma option flag").isNotEmpty()
        )
        assertTrue(
            "Flag without dash must be flagged",
            errorsContaining("#pragma option v+\n", "Invalid #pragma option flag").isNotEmpty()
        )
    }
    ...
    fun testOptionLetterNotValidForParserOption() {
        ...
        assertTrue(
            "An option letter must be rejected for parseroption",
            errorsContaining("#pragma parseroption -c+\n", "flag letter").isNotEmpty()
        )
    }
    ...
    fun testParserOptionLetterNotValidForOption() {
        ...
        assertTrue(
            "A parseroption letter must be rejected for option",
            errorsContaining("#pragma option -u+\n", "flag letter").isNotEmpty()
        )
    }

    fun testFlagsMissingArgumentProducesError() {
        assertTrue(
            "option without flags must be flagged",
            errorsContaining("#pragma option\n", "requires an argument").isNotEmpty()
        )
    }
    ...
    fun testVerboseLevelNonIntegerProducesError() {
        assertTrue(
            "A non-integer verboselevel must be flagged",
            errorsContaining("#pragma verboselevel \"x\"\n", "requires an integer value").isNotEmpty()
        )
    }

    fun testVerboseLevelOutOfRangeProducesError() {
        assertTrue(
            "verboselevel out of range must be flagged",
            errorsContaining("#pragma verboselevel 11\n", "between 0 and 10").isNotEmpty()
        )
    }

    fun testVerboseLevelMissingArgumentProducesError() {
        assertTrue(
            "verboselevel without argument must be flagged",
            errorsContaining("#pragma verboselevel\n", "requires an argument").isNotEmpty()
        )
    }
    ...
    fun testStringSubCommandWithIntegerProducesError() {
        assertTrue(
            "A non-string message must be flagged",
            errorsContaining("#pragma message 5\n", "requires a string value").isNotEmpty()
        )
    }

    fun testStringSubCommandMissingArgumentProducesError() {
        assertTrue(
            "message without argument must be flagged",
            errorsContaining("#pragma message\n", "requires an argument").isNotEmpty()
        )
    }
