...
/**
 * Tests for the `#if`/`#elif`/`#else`/`#endif` block-structure validation in [org.pcsoft.intellij.plugin.inno_setup.preprocessor.language.parser.IsPreprocessorAnnotator].
...
class IsPreprocessorConditionalStructureAnnotatorTest : IsTimedBasePlatformTestCase() {
    ...
    fun testNestedUnterminatedInnerIfProducesError() {
        val errors = structureErrors("#if 1\n#if 2\n#endif\n$setupTail")
        assertTrue(
            "The outer #if without #endif must be flagged: $errors",
            errors.any { it.contains("missing #endif") })
    }
}
