...
/**
 * Tests for the `#include` validation in [org.pcsoft.intellij.plugin.inno_setup.preprocessor.language.parser.IsPreprocessorAnnotator]: the value must be a string and a literal
...
class IsPreprocessorIncludeAnnotatorTest : IsTimedBasePlatformTestCase() {
    ...
    fun testMultiLineIncludedFileProducesNoWeakWarning() {
        ...
        assertTrue(
            "A multi-line included file must not be flagged as single-line",
            weakWarnings("single-line").isEmpty()
        )
    }
