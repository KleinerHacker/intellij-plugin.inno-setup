...
import org.pcsoft.intellij.plugin.inno_setup.preprocessor.language.parser.expression.IsPreprocessorExprValue.IntValue
import org.pcsoft.intellij.plugin.inno_setup.preprocessor.language.parser.expression.IsPreprocessorExprValue.StrValue
...
class IsPreprocessorExprEvaluatorTest {
    ...
    @Test
    fun `integer literal`() = assertEquals(IntValue(42), eval("42"))

    @Test
    fun `arithmetic respects precedence`() = assertEquals(IntValue(14), eval("2 + 3 * 4"))

    @Test
    fun `parentheses change precedence`() = assertEquals(IntValue(20), eval("(2 + 3) * 4"))

    @Test
    fun `string concatenation`() = assertEquals(StrValue("abcd"), eval("\"ab\" + \"cd\""))

    @Test
    fun `doubled quotes unescape`() = assertEquals(StrValue("a\"b"), eval("\"a\"\"b\""))

    @Test
    fun `reference is substituted`() =
        assertEquals(IntValue(10), eval("x + 1", mapOf("x" to IntValue(9))))

    @Test
    fun `comparison yields 0 or 1`() {
    ...
    @Test
    fun `division by zero is not computable`() = assertNull(eval("1 / 0"))

    @Test
    fun `function call is not computable`() = assertNull(eval("Len(\"x\")"))

    @Test
    fun `brace constant is not computable`() = assertNull(eval("{app}"))

    @Test
    fun `unresolved reference is not computable`() = assertNull(eval("y + 1"))

    @Test
    fun `mixing string and int is not computable`() = assertNull(eval("\"a\" + 1"))

    // ── value resolver (references between defines) ───────────────────────────

    @Test
    fun `value resolves transitively through defines`() {
    ...
    @Test
    fun `forward reference has no value`() {
    ...
    @Test
    fun `self reference terminates without value`() {
    ...
    @Test
    fun `function macro call is evaluated by substituting the argument`() {
    ...
    @Test
    fun `value flows through references and a macro call`() {
