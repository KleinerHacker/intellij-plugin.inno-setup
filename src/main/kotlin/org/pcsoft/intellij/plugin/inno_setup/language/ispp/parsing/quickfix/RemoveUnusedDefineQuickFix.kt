...
package org.pcsoft.intellij.plugin.inno_setup.language.ispp.parsing.quickfix
...
class RemoveUnusedDefineQuickFix : IntentionAction {
    ...
    constructor(directive: IsppDirective) {
        defineName = (directive as? IsppDirectiveEx)?.getDefineName().orEmpty()
    }
    ...
    override fun invoke(project: Project, editor: Editor?, file: PsiFile) {
        ...
        val issFile: IssFile = when (file) {
            is IssFile -> file
            else -> InjectedLanguageManager.getInstance(project).getTopLevelFile(file) as? IssFile ?: return
        }
        ...
        // injected languages where IsiIsppLine.textRange can be in injected coordinates).
