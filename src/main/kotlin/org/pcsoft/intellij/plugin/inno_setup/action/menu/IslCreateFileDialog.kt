 ...
 * [org.pcsoft.intellij.plugin.inno_setup.language.isi.completion.provider.IsiLanguageIdValueProvider]):
 ...
 */
class IslCreateFileDialog(project: Project) : DialogWrapper(project) {

    private val fileNameField = JBTextField()
    private val languageNameField = JBTextField()

    private val languages = service<IssLanguageService>().entries
    private val languageIdCombo = ComboBox(languages.toTypedArray()).apply {
        renderer = object : ColoredListCellRenderer<IslLanguageSpec>() {
            override fun customizeCellRenderer(
                list: JList<out IslLanguageSpec>,
                value: IslLanguageSpec?,
                index: Int,
                selected: Boolean,
                hasFocus: Boolean
            ) {
        ...
        addActionListener { (selectedItem as? IslLanguageSpec)?.let { languageNameField.text = it.displayName } }
    }

    val fileName: String get() = fileNameField.text.trim()
    val languageName: String get() = languageNameField.text.trim()
    val languageId: String get() = (languageIdCombo.selectedItem as? IslLanguageSpec)?.id.orEmpty()

    init {
        title = "New Inno Setup Language File"
        init()
        (languageIdCombo.selectedItem as? IslLanguageSpec)?.let { languageNameField.text = it.displayName }
    }
