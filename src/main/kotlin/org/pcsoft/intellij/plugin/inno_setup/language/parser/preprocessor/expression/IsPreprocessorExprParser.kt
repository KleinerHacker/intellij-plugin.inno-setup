...
class IsPreprocessorExprParser private constructor(
    ...
    private fun parse(): IsPreprocessorExprParseResult {
        if (tokens.isEmpty()) {
            return IsPreprocessorExprParseResult(IsPreprocessorExprEmpty(IsPreprocessorExprSpan(0, length)), emptyList())
        }
    ...
    private fun parsePrimary(): IsPreprocessorExprNode {
        ...
        return when (token.type) {
            IsPreprocessorExprTokenType.NUMBER -> { advance(); IsPreprocessorExprIntLiteral(token.span) }
            IsPreprocessorExprTokenType.STRING -> { advance(); IsPreprocessorExprStrLiteral(token.span) }
