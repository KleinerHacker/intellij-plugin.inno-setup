...
import org.pcsoft.intellij.plugin.inno_setup.language.isi.navigation.IsiLanguagePrefixReference
 ...
 */
abstract class IsiDirectiveEntryMixinImpl(node: ASTNode) : ASTWrapperPsiElement(node), IsiDirectiveEntryEx {
    ...
    override fun getNameIdentifier(): PsiElement? {
        if (!isCustomMessageDeclaration()) return null
        return (this as IsiDirectiveEntry).directiveKey.node.findChildByType(IsiTypes.IDENTIFIER)?.psi
    }
    ...
    override fun setName(name: String): PsiElement {
        ...
        val file = containingFile as? IssFile ?: return this
    ...
    override fun getReferences(): Array<PsiReference> {
        ...
        return arrayOf(IsiLanguagePrefixReference(entry, keyNode.text.substring(0, dot), TextRange(start, start + dot)))
    }

    override fun getTextOffset(): Int = getNameIdentifier()?.textOffset ?: super.getTextOffset()

    private fun renameKeyPart(entry: IsiDirectiveEntry, newName: String) {
        val idNode = entry.directiveKey.node.findChildByType(IsiTypes.IDENTIFIER) ?: return
        ...
        val dummy = PsiFileFactory.getInstance(project)
            .createFileFromText("dummy.iss", IssFileType.INSTANCE, "[CustomMessages]\n$newFull=x\n")
        val newId = PsiTreeUtil.findChildOfType(dummy, IsiDirectiveKey::class.java)
            ?.node?.findChildByType(IsiTypes.IDENTIFIER)?.psi ?: return
