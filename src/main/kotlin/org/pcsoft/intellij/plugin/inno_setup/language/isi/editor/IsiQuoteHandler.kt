...
class IsiQuoteHandler : TypedHandlerDelegate() {
    ...
    override fun beforeCharTyped(
        ...
        if (offset < text.length && text[offset] == '"') {
            ...
            if (!iter.atEnd() && iter.tokenType == IsiTypes.QUOTE && offset > 0) {
                val prev = editorEx.highlighter.createIterator(offset - 1)
                if (!prev.atEnd()) {
                    val pt = prev.tokenType
                    if (pt == IsiTypes.QUOTE || pt == IsiTypes.STRING_PART || pt == IsiTypes.RBRACE) {
        ...
        if (offset > 0) {
            ...
            if (!iter.atEnd()) {
                when (iter.tokenType) {
                    IsiTypes.STRING_PART, IsiTypes.RBRACE -> return Result.CONTINUE
                    IsiTypes.QUOTE ->
