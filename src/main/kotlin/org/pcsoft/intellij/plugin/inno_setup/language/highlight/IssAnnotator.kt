...
import org.pcsoft.intellij.plugin.inno_setup.language.psi.IssSection
...
class IssAnnotator : Annotator {
    ...
    private fun annotateKey(range: TextRange, attr: IssAttribute?, holder: AnnotationHolder) {
        when {
            attr == null ->
    ...
    private fun annotateParamValue(value: IssParamValue, holder: AnnotationHolder, spec: InnoSetupSpec) {
        ...
        when (val type = attr.type) {
            is IssFlagType    -> annotateFlagValue(value, type, holder)
            is IssNativeType  -> annotateNativeValue(value, type, holder)
            is IssReferenceType -> {
    ...
    private fun resolveAttr(value: IssParamValue, spec: InnoSetupSpec): IssAttribute? {
        val pair = value.containingParamPair()
        if (pair != null) {
    ...
    private fun annotateFlagValue(value: IssParamValue, flagType: IssFlagType, holder: AnnotationHolder) {
        ...
        tokenNodes.forEach { (name, node) ->
            ...
            def.conflicts.forEach conflict@{ conflict ->
                ...
                val severity = if (conflict.severity == IssFlagSeverity.ERROR)
    ...
    private fun annotateNativeValue(value: IssParamValue, type: IssNativeType, holder: AnnotationHolder) {
