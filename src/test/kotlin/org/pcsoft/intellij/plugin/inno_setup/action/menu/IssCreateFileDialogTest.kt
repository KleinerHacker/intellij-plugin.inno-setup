...
/**
 * Tests for [IssCreateFileDialog]'s exposed input properties (the values consumed by
 * [IssCreateFileAction.buildTemplate]). The dialog is never shown; only its model is exercised.
 */
class IssCreateFileDialogTest : BasePlatformTestCase() {

    private fun withDialog(block: (IssCreateFileDialog) -> Unit) {
    ...
    private fun setField(dialog: IssCreateFileDialog, fieldName: String, text: String) {
        val field = IssCreateFileDialog::class.java.getDeclaredField(fieldName).apply { isAccessible = true }
    ...
    fun testEnglishIsPreselected() = withDialog { dialog ->
        val english = service<IssLanguageService>().fromIssName("english")
