...
/** Tests for the Windows-LCID reference data (isl-code.yaml) backing LanguageID completion/validation/inlays. */
class IssLanguageServiceTest : BasePlatformTestCase() {

    private val service get() = service<IssLanguageService>()

    fun testParseIdHandlesHexDecimalAndZero() {
        assertEquals(0x0409, IssLanguageService.parseId("\$0409"))
        assertEquals(0, IssLanguageService.parseId("0"))
        assertEquals(0, IssLanguageService.parseId("\$0"))
        assertEquals(1033, IssLanguageService.parseId("1033")) // decimal == 0x0409
    }

    fun testParseIdReturnsNullForMalformed() {
        assertNull(IssLanguageService.parseId("\$GG"))
        assertNull(IssLanguageService.parseId("abc"))
        assertNull(IssLanguageService.parseId(""))
    }
    ...
    fun testEveryCuratedEntryHasParseableIdInValidSet() {
        service.entries.forEach { entry ->
            val numeric = IssLanguageService.parseId(entry.id)
