...
/**
 * Tests for [IssFlipIntentionAction], which swaps the parameter pair before the caret with the
 * following one inside a parameter section entry. The caret is expected to sit directly behind the
 * `;` separating the two pairs.
 */
class IssFlipIntentionActionTest : BasePlatformTestCase() {

    private val action = IssFlipIntentionAction()
