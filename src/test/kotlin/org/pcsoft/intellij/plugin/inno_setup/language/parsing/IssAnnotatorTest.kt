...
/**
 * Comprehensive tests for IssAnnotator — one test per defined case.
 *
 * Existing tests in IssAnnotatorIsppTest and IssHighlightingTest cover ISPP constant
 * and section-name highlighting; this file covers the remaining annotator branches.
 */
class IssAnnotatorTest : BasePlatformTestCase() {
    ...
    fun testFileLevelErrorWhenSetupSectionMissing() {
        ...
        val hit = all.any { it.severity == HighlightSeverity.ERROR &&
    ...
    fun testFilesEntryWithAllRequiredParamsProducesNoEntryError() {
        val text = VALID_SETUP + "\n[Files]\nSource: \"app.exe\"; DestDir: \"{app}\"\n"
        val all = highlights(text)
        val entryErrors = all.filter { it.severity == HighlightSeverity.ERROR &&
            it.description?.contains("Required parameter", ignoreCase = true) == true }
        assertTrue("Complete [Files] entry must not produce 'Required parameter' ERROR",
            entryErrors.isEmpty())
    }
    ...
    fun testUnknownDirectiveKeyProducesError() {
        ...
        val all = highlights(text)
        val hit = all.any { it.severity == HighlightSeverity.ERROR &&
            it.forcedTextAttributesKey == IssAnnotatorHighlighting.UNKNOWN_REFERENCE }
    ...
    fun testValidFlagHighlightedAsFlagColor() {
        ...
        val hit = all.any { it.forcedTextAttributesKey == IssAnnotatorHighlighting.FLAG &&
    ...
    fun testConflictingFlagsWithErrorSeverityProducesTwoErrors() {
        ...
        val all = highlights(text)
        val conflicts = all.filter { it.severity == HighlightSeverity.ERROR &&
            it.description?.contains("Conflicting flags", ignoreCase = true) == true }
        assertEquals("Conflicting flags 'deleteafterinstall'/'restartreplace' must produce exactly 2 ERROR annotations",
            2, conflicts.size)
    }
    ...
    fun testConflictingFlagsWithWarningSeverityProducesTwoWarnings() {
        ...
        val all = highlights(text)
        val conflicts = all.filter { it.severity == HighlightSeverity.WARNING &&
            it.description?.contains("Conflicting flags", ignoreCase = true) == true }
        assertEquals("Conflicting flags 'ignoreversion'/'comparetimestamp' must produce exactly 2 WARNING annotations",
            2, conflicts.size)
    }
    ...
    fun testValidBooleanValueProducesNoTypeError() {
        ...
        val all = highlights(text)
        val typeErrors = all.filter { it.severity == HighlightSeverity.ERROR &&
            it.description?.contains("Expected type", ignoreCase = true) == true }
        assertTrue("Valid boolean value 'yes' for CloseApplications must not produce type ERROR",
            typeErrors.isEmpty())
    }
    ...
    fun testValidIntegerValueProducesNoTypeError() {
        ...
        val typeErrors = all.filter { it.severity == HighlightSeverity.ERROR &&
            it.description?.contains("Expected type", ignoreCase = true) == true }
        assertTrue("Valid integer value '1024' for ExtraDiskSpaceRequired must not produce type ERROR",
            typeErrors.isEmpty())
    }
    ...
    fun testKnownBuiltinConstantHighlightedAsReference() {
        ...
        val hit = all.any { it.forcedTextAttributesKey == IssAnnotatorHighlighting.REFERENCE &&
