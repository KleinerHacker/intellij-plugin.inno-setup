...
import org.pcsoft.intellij.plugin.inno_setup.language.parsing.psi.IssParameterEntry
 ...
 * [IssStructureViewModel] / [IssStructureViewFactory].
 */
class IssStructureViewTest : BasePlatformTestCase() {
    ...
    fun testFileChildrenAreSections() {
        ...
        children.forEach { assertTrue(it is IssStructureViewElement && it.value is IssSection) }
    }

    fun testParameterSectionExposesEntryChildren() {
        val file = issFile()
        val filesSection = IssStructureViewElement(file).children
            .first { (it as IssStructureViewElement).value.let { v -> v is IssSection && v.nameTextEquals("Files") } }
        ...
        assertTrue((entries[0] as IssStructureViewElement).value is IssParameterEntry)
    ...
    fun testDirectiveSectionHasNoChildren() {
        ...
        val setupSection = IssStructureViewElement(file).children
            .first { (it as IssStructureViewElement).value.let { v -> v is IssSection && v.nameTextEquals("Setup") } }
    ...
    fun testFactoryReturnsNullForNonIssFile() {
        ...
        assertNull(IssStructureViewFactory().getStructureViewBuilder(txt))
    }

    private fun IssSection.nameTextEquals(name: String): Boolean =
