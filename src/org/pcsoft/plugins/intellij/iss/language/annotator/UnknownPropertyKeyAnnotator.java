    ...
    public void annotate(@NotNull PsiElement psiElement, @NotNull AnnotationHolder annotationHolder) {
        if (psiElement instanceof IssKey) {
            ...
            if (sectionType == null || sectionType == SectionType.Messages || sectionType == SectionType.CustomMessages)
                return;
            final SectionProperty[] sectionPropertyEnumList = sectionType.getSectionValueClass().getEnumConstants();

            if (Stream.of(sectionPropertyEnumList).noneMatch(sectionValue -> sectionValue.getName().equalsIgnoreCase(key.getName()))) {
                annotationHolder.createErrorAnnotation(psiElement, "Unable to find key for section " + section.getSectionTitle().getName() + ", allowed are " +
                        Arrays.toString(Stream.of(sectionPropertyEnumList).map(SectionProperty::getName).toArray())).setHighlightType(ProblemHighlightType.LIKE_UNKNOWN_SYMBOL);
