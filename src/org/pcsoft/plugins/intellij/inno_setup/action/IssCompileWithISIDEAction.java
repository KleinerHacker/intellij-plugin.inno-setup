 ...
 */
public class IssCompileWithISIDEAction extends AnAction {
    private static final IssCompilerSettings COMPILER_SETTINGS = ServiceManager.getService(IssCompilerSettings.class);

    public IssCompileWithISIDEAction() {
        super("Compile extern with Inno Setup IDE", "Open the selected file with Inno Setup IDE and compile immediate.", IssIcons.IS_CMD);
    }
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        try {
            Runtime.getRuntime().exec("\"" + ideFile.getAbsolutePath() + "\" /cc \"" + virtualFile.getPath() + "\"");
        } catch (IOException e1) {
            Logger.getInstance(IssCompileWithISIDEAction.class).error("Unable to start Inno Setup IDE", e1);
