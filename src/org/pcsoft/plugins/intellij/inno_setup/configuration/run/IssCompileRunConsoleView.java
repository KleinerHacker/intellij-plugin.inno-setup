 ...
 */
public class IssCompileRunConsoleView extends ConsoleViewImpl {
    private final Consumer<String> standardOutCallback, errorOutCallback;

    public IssCompileRunConsoleView(Project project, Consumer<String> standardOutCallback, Consumer<String> errorOutCallback) {
        super(project, true);
        ...
        addMessageFilter((line, offsetWithLine) -> {
            ...
            if (line.toLowerCase().startsWith("run:") || line.startsWith("Inno Setup finished with exit code") || line.equals("Inno Setup was detached")) {
                return new Filter.Result(startOffset, endOffset, null, IssCompileRunHighlightingColorFactory.CONSOLE_OUT_IDE.getDefaultAttributes());
            } else if (IssCompileRunErrorHandler.hasError(line.trim(), IssCompileRunErrorHandler.ErrorType.CodeLine)) {
                return new Filter.Result(startOffset + line.indexOf(" in ") + 4, startOffset + line.lastIndexOf(":"), new HyperlinkInfoBase() {
                    ...
                    public void navigate(Project project, RelativePoint relativePoint) {
                        IssCompileRunErrorHandler.handleError(project, line.trim());
                    }
                });
            }
