...
public abstract class IssAbstractSectionAnnotator<E extends IssDefinitionElement> implements Annotator {
    ...
    public final void annotate(@NotNull final PsiElement psiElement, @NotNull final AnnotationHolder annotationHolder) {
        if (elementClass.isAssignableFrom(psiElement.getClass())) {
            ...
            checkCommonProperties(definitionElement, annotationHolder);
    ...
    private void checkCommonProperties(@NotNull E definitionElement, @NotNull final AnnotationHolder annotationHolder) {
        for (final IssPropertyElement propertyElement : (Collection<IssPropertyElement>)definitionElement.getDefinitionPropertyList()) {
            if (propertyElement instanceof IssPropertyLanguagesReferenceElement) {
                final IssPropertyLanguagesReferenceElement languagesElement = (IssPropertyLanguagesReferenceElement) propertyElement;
                languagesElement.getPropertyValueList().stream()
                        .filter(valueElement -> valueElement.getReference().resolve() == null)
            ...
            } else if (propertyElement instanceof IssPropertyWindowsVersionElement) {
                final IssPropertyWindowsVersionElement windowsVersionElement = (IssPropertyWindowsVersionElement) propertyElement;
        ...
        for (final IssPropertyElement<?> propertyElement : (Collection<IssPropertyElement>) definitionElement.getDefinitionPropertyList()) {
            ...
            switch (propertyElement.getPropertyType().getValueType()) {
                ...
                case String:
                    ...
                    if (PsiTreeUtil.getChildOfType(propertyElement.getPropertyValue(), IssStringElement.class) == null) {
