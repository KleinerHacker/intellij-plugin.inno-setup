...
| 5 | ~~`#ifexist` / `#ifnexist` never checks the file.~~ **Done:** the path is resolved through `IsIncludePaths` and decides the branch; only a non-literal path stays undecidable.                                                                                                     | ✅      | `IsPreprocessorBranchAnalysis` (`evaluateBranchCondition`)               |
...
| 7 | ~~No conditional compilation in the editor.~~ **Done:** `#if` branches are evaluated three-valued (active/inactive/undecidable); provably dead branches are dimmed and stop reporting problems, undecidable conditions get a weak-warning marker, and the effective script prunes accordingly. Surfacing `#define` values as inlay/Quick Doc is still open. | ⚠️     | `IsPreprocessorBranchAnalysis`, `IsEffectiveScriptBranches`              |
...
| 9 | **ISCC command-line symbols (`/D…`) are only considered for branch analysis.** A run configuration's `/D` symbols feed the conditional evaluation via `IsPreprocessorSymbolProvider`, but plain `{#Name}` references to them are still reported as unresolved.                     | ⚠️     | `IsPreprocessorSymbolProvider`, `IsRunConfigurationSymbolProvider`        |
