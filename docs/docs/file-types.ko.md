...
`.ist` 파일은 ISPP의 `#include` 지시문을 통해 스크립트에 가져오기 위한 재사용 가능한 자유 텍스트 조각입니다. 템플릿은 전체 섹션, 부분 섹션, 흩어진 지시문 줄 또는 전처리기 정의를 담을 수 있으며, 완전한 설치 프로그램이 아니라 구성 요소일 뿐이므로 단독으로 검증되지 않습니다. 그 내용은 호스트 `.iss`/`.isl` 스크립트에 포함된 후에야 분석됩니다.
