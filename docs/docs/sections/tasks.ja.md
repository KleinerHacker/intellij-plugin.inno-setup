...
`[Tasks]` セクションは、ユーザーが*追加タスクの選択*
ウィザードページで有効または無効にできるオプションアクションを定義します——デスクトップショートカットの作成やコンテキストメニューエントリーの追加など。タスクはチェックボックスとして表示され、グループ内で
`exclusive` とマークされた場合はラジオボタンとして表示されます。他のセクションのエントリーは `Tasks`
パラメーターを通じてタスクにリンクされます。
