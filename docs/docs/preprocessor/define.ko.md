...
`#define`은 전처리기 매크로 — 컴파일 시점에 스크립트로 치환되는 명명된 값 또는 표현식 — 를 선언합니다. 가장 많이 사용되는 ISPP 지시문이며, 플러그인이 완전한 의미 체계(참조 해석, 이름 바꾸기,
사용 찾기)로 지원하는 지시문입니다.
...
- `{#Name}`은 해당 `#define` 선언으로 해석됩니다 — 정의로 이동(**Ctrl+B** / **Cmd+B**)과 사용 찾기(**Alt+F7**)가 작동하며, 이름 바꾸기는 선언과 모든 사용을
  동기화합니다.
...
`#define`의 값은 단순한 리터럴이 아니라 완전한 **식**입니다. ISPP는 C/C++ 스타일의 식 문법을 사용하여 컴파일 시점에 평가하며, 플러그인은 이를 파싱·타입 검사·강조 표시합니다. 단일 리터럴은
가장 단순한 경우이며, 여러 값을 결합하려면 **연산자**로 연결해야 합니다.
...
`#define` 값은 임의의 ISPP **내장 함수**를 호출할 수 있으며(공식 전체 집합이 플러그인에 포함되어 있습니다), 함수의 반환 타입이 주변 식으로 흘러 들어갑니다 — 예를 들어 `Str(...)`은
`str`, `Int(...)`와 `Power(...)`는 `int`, `FileExists(...)`는 `int`입니다. 따라서 `Str(Major) + "."`은 유효하지만(`str` + `str`),
`Str(Major) * 2`는 거부됩니다.
...
### 내장 함수 호출은 시그니처로 검사됩니다

내장 함수 호출은 포함된 시그니처(아래 레퍼런스)에 대해 검증됩니다:

| 예                                   | 이유                                                    |
|--------------------------------------|---------------------------------------------------------|
| `#define X Copy("abc")`              | `Copy`는 인수 3개가 필요합니다                            |
| `#define X Copy("abc", "x", 2)`      | `Index`는 `int`로 선언되었지만 문자열이 전달되었습니다      |
| `#define X StringChange("lit", …)`   | 참조 전달 매개변수(`S: str*`)에는 매크로 이름이 필요합니다  |
| `#define X Warning("x") + 1`         | `Warning`은 값을 반환하지 않아 식에 쓸 수 없습니다          |
| `#define X NoSuchFunc(1)`            | 알 수 없는 전처리기 함수                                  |

기본값이 있는 매개변수(예: `Find(S, Substr, Index = 1)`)는 생략할 수 있으므로, 필수 매개변수가 모두 전달되면
인수가 적은 호출도 허용됩니다.

`Ident` 또는 `Array`로 표시된 매개변수(`Defined(Ident)`, `TypeOf(Ident)`, `DimOf(Array)`)는 값이 아니라 심볼의
**이름**을 받습니다: 인수는 순수한 식별자여야 합니다. `Defined`/`TypeOf`에서는 정의되지 않아도 되지만,
`DimOf`에서는 배열이 존재해야 하며 인덱스 없이 전달합니다.

```ini
#dim Langs[2]
#define HasDebug  Defined(DEBUG)   ; DEBUG는 존재하지 않아도 됩니다
#define Count     DimOf(Langs)     ; Langs[0]이 아니라 배열 자체
```

### 호출을 입력하는 동안의 매개변수 정보

인수 목록 안에 캐럿을 두고 **Ctrl+P**(매개변수 정보)를 누르면 호출 대상의 매개변수 목록이 표시되고 지금 입력 중인
인수가 강조됩니다:

* **내장 함수**는 시그니처를 그대로 보여줍니다 — `S: str, Index: int, Count: int → str`. 참조 전달을 뜻하는 `*`와
  선택적 매개변수의 기본값도 포함됩니다.
* **함수형 매크로**는 타입, `*`, 기본값이 선언되어 있으면 그 선언을 그대로 보여줍니다
  (`#define Multiply(int A, int B = 10)` → `A: int, B: int = 10 → int`). 선언이 없는 매개변수는 매크로 본문에서
  추론된 타입을 표시하며 (`#define Pad(n) "0" + Str(n)` → `n: int → str`), 판단할 수 없으면 매개변수 이름만
  표시합니다.
...
해결은 **뒤쪽** 참조만 따릅니다(매크로는 이전 줄에 이미 선언되어 있어야 함). 이로써 올바르게 작성된 스크립트에서는 참조 순환이 불가능합니다. 잔여 순환(예: 자기 참조 `#define P P + 1` 또는
순서가 어긋난 상호 참조)은 안전하게 끊어집니다: 참조가 `any`로 축소되므로 무한 루프도 잘못된 오류도 발생하지 않습니다.
...
식 규칙은 함수형 매크로 본문에도 적용됩니다. 타입이 선언되지 않은 매개변수는 본문이 제약하지 않는 한 `any`입니다 —
`x * 2`는 정수에서만 성립하므로 `x`는 `int`로 처리됩니다:
...
본문 안에서 매개변수는 스코프에 있습니다: 코드 완성에 제공되고, **Ctrl+B**로 사용처에서 매개변수 목록의 선언으로
이동하며, 사용처 찾기가 동작하고, **Shift+F6**은 매크로 안의 매개변수만 일괄 이름 변경합니다 — 매크로 이름은
바뀌지 않습니다.

### 매크로 매개변수: 타입, 참조 전달, 기본값

ISPP는 매개변수를 `[<타입>] [*]<이름> [= <기본값>]` 형태로 선언하며, `<타입>`은 `any`, `int`, `str`, `func`입니다.
플러그인은 이 선언을 해석하여 모든 호출을 검사합니다:
...
| 선언              | 의미                                                              |
|-------------------|-------------------------------------------------------------------|
| `int A`           | 인수는 정수 호환이어야 하며, 그렇지 않으면 오류로 표시됩니다      |
| `str S`           | 인수는 문자열 호환이어야 합니다                                   |
| `any V` / 타입 없음 | 모든 인수를 허용합니다(타입이 없으면 본문이 좁힐 수 있습니다)     |
| `func F`          | 매크로를 값으로 갖는 매개변수 — `any`처럼 허용됩니다               |
| `B = 10`          | 선택적: 인수를 생략할 수 있고 값에는 기본값이 사용됩니다          |
| `*Rest`           | 참조 전달: 인수는 ISPP가 되돌려 쓸 수 있는 매크로 이름이어야 합니다 |

따라서 호출은 내장 함수 호출과 똑같이 검사됩니다. 인수 개수는 필수 매개변수 수와 선언된 매개변수 수 사이여야 하고,
각 인수는 해당 매개변수의 타입에 맞아야 합니다.

```ini
#define X  Multiply(2)          ; 정상 — B는 기본값 10
#define Y  Multiply()           ; 오류: 1~2개의 인수가 필요하지만 0개
#define Z  Multiply("a", 2)     ; 오류: 인수 1('A')은 int여야 하지만 str
#define W  Split("a,b", "lit")  ; 오류: 인수 2('Rest')는 참조로 전달됩니다
```

선언된 타입, `*`, 기본값은 매개변수 정보 팝업(**Ctrl+P**)과 매크로의 빠른 문서(**Ctrl+Q**)에도 표시됩니다.

---

## 인레이 힌트로 표시되는 계산 값

`#define`이 다른 매크로와 연산자로 조합된 경우 플러그인이 이를 평가하여 결과를 줄 끝에 회색 **인레이 힌트**로
표시합니다. 스크립트를 컴파일하지 않고도 조합된 값을 확인할 수 있습니다:

```ini
#define MyName    "MyApp"
#define MyVer     "1.5"
#define MyExe     MyName + ".exe"          ; 힌트: "MyApp.exe"
#define MyTitle   MyName + " " + MyVer     ; 힌트: "MyApp 1.5"
#define Build     Major * 100 + Minor      ; 힌트: 203
```

직접 정의한 함수형 매크로의 호출도 인수를 바인딩하여 평가됩니다:

```ini
#define ExeOf(str Name)  Name + ".exe"
#define MyExe            ExeOf("MyApp")    ; 힌트: "MyApp.exe"
```

*계산된* 값만 표시됩니다. 다음 경우에는 힌트가 표시되지 않습니다:

- 단순 리터럴 (`#define MyVer "1.5"`) — 값이 이미 소스에 그대로 있으므로,
- 함수형 매크로 자체 — 결과가 호출마다 인수에 따라 달라지므로,
- 배열 요소 정의 (`#define Names[0] …`) 및 값이 없는 `#define`,
- 분석 시점에 평가할 수 없는 식 — 해결되지 않은 참조, `{…}` 상수, ISPP **내장 함수** 호출. 플러그인은 내장
  함수 호출을 시그니처와 대조해 검사하지만 실행하지는 않으므로 `Str` 이 순수하더라도
  `#define S Str(Major)` 에는 힌트가 표시되지 않습니다.

같은 값은 매크로의 빠른 문서 (**Ctrl+Q**)에서도 확인할 수 있습니다. 힌트는
**Settings | Editor | Inlay Hints**에서 끌 수 있습니다.

---

## 내장 함수 참조

ISPP는 `#define` 식 안에서 호출할 수 있는 많은 **내장 함수**를 제공합니다. 플러그인에는 공식 전체 집합이 포함되어 있으며, 각 함수의 반환 형식은 식 형식 검사기에 공급되고(위 참조) 완성에도
제시됩니다. 아래 목록은 빠짐없으며 (공식 ISPP 함수 인덱스에 맞춰) 알파벳순으로 정렬되어 있습니다.
...
직접 만든 `#define` 외에도 ISPP는 선언 없이 사용할 수 있는 **사전 정의 변수** 집합을 제공합니다. **값을 가진** 변수(`int` / `str`)는 사용자 정의와 마찬가지로 `{#…}`로 인라인
출력할 수 있으며 식에서도 사용할 수 있습니다. **값이 없는** 변수(`void`)는 조건부 컴파일 전용입니다. 아래 목록은 완전합니다:
...
이들은 `{#…}` 완성에 표시되고 검증에서 허용됩니다. 경로 관련 변수(`{#SourcePath}`, `{#__DIR__}`, `{#CompilerPath}`, `{#SysPath}`)는 플러그인이
`[Languages]`의 `MessagesFile` 경로를 해석할 때도 확장됩니다. 나머지 동적 변수는 거짓 오류를 생성하지 않고 해석되지 않은 상태로 남습니다.

!!! note "값이 없는 기호"
`__WIN32__`, `ISPP_INVOKED`, `ISCC_INVOKED`, `WINDOWS`, `UNICODE`는 **값이 없습니다**: 이들은 조건부 컴파일(`#ifdef` /
`#if defined(...)`)을 위해서만 *정의*되므로 `{#…}`로 출력할 **수 없습니다**. `{#…}` 완성에서 제외되고 인라인 출력으로 허용되지 않습니다.
