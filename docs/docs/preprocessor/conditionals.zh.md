...
命令行接收符号。只要结果不确定，**两个分支都会保持正常显示**，并在条件上标注蓝色波浪下划线和原因提示。绝不会靠猜测去淡化。
...
保留，并附上说明原因的注释。被删除的分支留下的空行会被整理：删除处最多保留一个空行，脚本开头和结尾则
不保留。
...
- **`#ifexist` / `#ifnexist`** —— 文件名是完整的 ISPP **字符串**表达式：会验证运算符和类型错误（值必须是
  字符串），标识符会解析到其 `#define`。该表达式还会被**求值**（字面路径或由 `#define` 拼接而成的路径均
  可），并根据该文件是否存在来判定分支。（这些指令可以测试磁盘上的*任意*文件，而不仅是脚本文件，因此不提供
  文件名补全。）
