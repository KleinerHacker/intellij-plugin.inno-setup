...
- **내용 인라인 미리 보기** — 포함된 파일의 해석된 내용이 `#include` 줄 아래에 인레이 힌트로(파일의
  줄바꿈을 유지하여) 표시되며, 클릭하면 해당 파일로 이동합니다. [편집기 설정](../settings-editor.md)
  페이지에서 켜고 끌 수 있습니다(기본 활성화). 파일을 찾을 수 없거나 읽을 수 없으면 미리 보기가
  표시되지 않습니다.
