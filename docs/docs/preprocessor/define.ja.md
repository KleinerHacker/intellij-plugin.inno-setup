...
引数リスト内にキャレットを置いて **Ctrl+P**（パラメータ情報）を押すと、呼び出し先のパラメータ一覧が表示され、
入力中の引数が強調されます：

* **組み込み関数**ではシグネチャがそのまま表示されます——`S: str, Index: int, Count: int → str`。参照渡しの `*` や
  省略可能パラメータの既定値も含まれます。
* **関数形式マクロ**では、型・`*`・既定値が宣言されていればその宣言がそのまま表示されます
  （`#define Multiply(int A, int B = 10)` → `A: int, B: int = 10 → int`）。宣言のないパラメータは
  マクロ本体から推論できた型が表示され（`#define Pad(n) "0" + Str(n)` → `n: int → str`）、
  判定できない場合はパラメータ名のみが表示されます。
...
式の規則は関数形式マクロの本体にも適用されます。型宣言のないパラメータは、本体が制約しない限り `any` です——
`x * 2` は整数でしか成立しないため、`x` は `int` として扱われます：
...
本体の中ではパラメータがスコープ内にあります：補完に提示され、**Ctrl+B** で使用箇所からパラメータリストの宣言へ
ジャンプでき、使用箇所検索も機能し、**Shift+F6** はマクロ内のパラメータのみを一括で改名します——マクロ名は変わりません。

### マクロのパラメータ：型・参照渡し・既定値

ISPP はパラメータを `[<型>] [*]<名前> [= <既定値>]` の形で宣言します。`<型>` は `any`、`int`、`str`、`func` です。
プラグインはこの宣言を解析し、すべての呼び出しを検証します：
...
| 宣言              | 意味                                                             |
|-------------------|------------------------------------------------------------------|
| `int A`           | 引数は整数互換でなければならず、そうでなければエラー             |
| `str S`           | 引数は文字列互換でなければならない                               |
| `any V` / 型なし  | すべての引数を受け付ける（型なしの場合は本体が絞り込むことがある）|
| `func F`          | マクロを値とするパラメータ——`any` と同様に扱われる                |
| `B = 10`          | 省略可能：引数を省略でき、値には既定値が使われる                 |
| `*Rest`           | 参照渡し：引数は ISPP が書き戻せるマクロ名でなければならない      |

したがって呼び出しは組み込み関数と同じように検証されます。引数の個数は必須パラメータ数と宣言済みパラメータ数の
間でなければならず、各引数はパラメータの型に適合する必要があります。

```ini
#define X  Multiply(2)          ; OK——B は既定値 10
#define Y  Multiply()           ; エラー：1〜2 個の引数が必要ですが 0 個
#define Z  Multiply("a", 2)     ; エラー：引数 1（'A'）は int が必要ですが str
#define W  Split("a,b", "lit")  ; エラー：引数 2（'Rest'）は参照渡し
```

宣言された型、`*`、既定値はパラメータ情報ポップアップ（**Ctrl+P**）とマクロのクイックドキュメント
（**Ctrl+Q**）にも表示されます。
...
`#define` が他のマクロと演算子から組み立てられている場合、プラグインはそれを評価し、結果を行末にグレーの
**インレイヒント** として表示します。スクリプトをコンパイルしなくても、組み立てられた値を確認できます:

```ini
#define MyName    "MyApp"
#define MyVer     "1.5"
#define MyExe     MyName + ".exe"          ; ヒント: "MyApp.exe"
#define MyTitle   MyName + " " + MyVer     ; ヒント: "MyApp 1.5"
#define Build     Major * 100 + Minor      ; ヒント: 203
```

自分で定義した関数形式マクロの呼び出しも、引数を束縛して評価されます:

```ini
#define ExeOf(str Name)  Name + ".exe"
#define MyExe            ExeOf("MyApp")    ; ヒント: "MyApp.exe"
```

表示されるのは *計算された* 値だけです。次の場合はヒントが表示されません:

- 単純なリテラル (`#define MyVer "1.5"`) — 値はすでにソースに書かれているため、
- 関数形式のマクロ自体 — 結果は呼び出しごとの引数に依存するため、
- 配列要素の定義 (`#define Names[0] …`) および値を持たない `#define`、
- 解析時に評価できない式 — 未解決の参照、`{…}` 定数、および ISPP **組み込み関数** の呼び出し。プラグインは
  組み込み関数の呼び出しをシグネチャに照らして検査しますが、実行はしません。そのため `Str` が純粋であっても
  `#define S Str(Major)` にはヒントが付きません。

同じ値はマクロのクイックドキュメント (**Ctrl+Q**) でも確認できます。ヒントは
**Settings | Editor | Inlay Hints** で無効にできます。
